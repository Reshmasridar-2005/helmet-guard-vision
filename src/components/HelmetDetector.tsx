...
const HelmetDetector = () => {
  ...
  const analyzeFrame = useCallback(async () => {
    ...
    try {
      ...
      // Look for person and check for helmet-like objects
      const person = results.find((r: any) => r.label === 'person');
      const helmetObjects = results.filter((r: any) => 
        r.label.includes('hat') || 
        r.label.includes('helmet') || 
        r.score > 0.7
      );

      const hasHelmet = person && helmetObjects.length > 0;
      const confidence = person ? Math.max(...helmetObjects.map((h: any) => h.score), 0.5) : 0;
      ...
      // Fallback: simple motion detection for demo
      const hasMotion = Math.random() > 0.3;
      setDetectionResult({
        hasHelmet: hasMotion,
        confidence: hasMotion ? 0.8 : 0.2,
        timestamp: new Date()
      });
    }
  }, [detector, accessGranted, toast]);
    ...
    <div className="w-full max-w-4xl mx-auto space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground">
          AI-powered helmet detection for mine worker safety compliance
        </p>
      </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-medium">
                        {detectionResult.hasHelmet ? 'Helmet Detected' : 'No Helmet Detected'}
                      </span>
                    </div>
                    <Badge variant={detectionResult.hasHelmet ? "default" : "destructive"}>
